 ...
 */
function testNullableGenericListWithCallback(callable $formatter, ?array $items = null): array
{
    if ($items === null) {
        return [];
    }
...
describe('Generic Callables (@template T with callable(T): T)', function () {
    ...
    describe('Standalone Generic Callables with Multiple Template Arguments', function () {
        ...
        test('pre-infers template T from nullable generic array before wrapping callback', function () {
            $intFormatter = fn (int $x): string => "val_{$x}";
            $res = testNullableGenericListWithCallback($intFormatter, [10, 20]);
            expect($res)->toBe(['val_10', 'val_20']);

            expect(fn () => testNullableGenericListWithCallback($intFormatter, [10, 'invalid_item']))
                ->toThrow(TypeError::class, '[1]')
            ;

            expect(testNullableGenericListWithCallback($intFormatter, null))->toBe([]);
        });
    });
